...
use crate::{
    ...
    keys::{export_sk_bip38, AccountKeys}, network::Network, types::TransparentAccountInfo,
};
...
pub fn derive_zip32_keys(network: &Network, connection: &Connection, account: u32, 
    acc_index: u32, addr_index: u32) -> Result<ZIP32KeysT> {
    let ai = get_account_info(network, connection, account)?;
    let Some(phrase) = ai.seed else { anyhow::bail!("No Seed") };
    ...
    let taddress = ak.tvk.as_ref().map(|tvk| 
        TransparentAccountInfo::derive_address(tvk, addr_index).encode(network)
    );
    let zip32 = ZIP32KeysT {
        tsk,
        taddress,
        zsk: ak.ssk.as_ref().map(|sk| encode_extended_spending_key(network.hrp_sapling_extended_spending_key(), &sk)),
...
pub fn find_address_index(sapling: &DiversifiableFullViewingKey, start: u32) -> u32 {
    let (di, _) = sapling.find_address(
        DiversifierIndex::from(start)).unwrap();
